def generateField(trans,medium,field,iscomplete=0):
    ...
    OUTPUT ARG
        pressure_field == [2D Pandas Data Frame][p/p0] Resulting Rayleigh Integral Pressure Field
        z_axis == [1D Pandas Data Frame][m] Axial axis of Pressure Field
        r_axis == [1D Pandas Data Frame][m] Radial axis of Pressure Field 
    ...
    df_pressure2D = pd.DataFrame(pressure_field)
    z_axis = pd.DataFrame(z_values)
    r_axis = pd.DataFrame(r_values)

    print('Rayleigh Integral Complete')
    iscomplete = 1
    return df_pressure2D, z_axis, r_axis, iscomplete